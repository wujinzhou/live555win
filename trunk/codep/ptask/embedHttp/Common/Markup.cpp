...
bool CMarkup::LoadXml( const char* szXml )
{
	...
	if ( nFileByteLen )
	{
		...
		MCD_CHAR* pUTF16Buffer = MCD_GETBUFFER(m_strDoc,nWideLen);
		...
		*/
		...
		MCD_RELEASEBUFFER( m_strDoc, pUTF16Buffer, nWideLen );
	}
	...
	if ( nFileByteLen )
	{
		MCD_CHAR* pUTF8Buffer = MCD_GETBUFFER(m_strDoc,nFileByteLen);
		
		MCD_PSZCPY( pUTF8Buffer , szXml );

		MCD_RELEASEBUFFER( m_strDoc, pUTF8Buffer, nFileByteLen );
#if defined(_MBCS) // needs to be in memory as MBCS
		MCD_STR strEncoding = GetDeclaredEncoding( m_strDoc );

		//修改默认编码采用ANSI.不采用原来的UTF8
		//if ( MCD_STRISEMPTY(strEncoding) || MCD_PSZNICMP(MCD_2PCSZ(strEncoding),_T("UTF-8"),5)==0 )
			//m_strDoc = UTF8ToA( m_strDoc );
		
		if ( MCD_PSZNICMP(MCD_2PCSZ(strEncoding),_T("UTF-8"),5)==0 )
			m_strDoc = UTF8ToA( m_strDoc );
#endif
	}
